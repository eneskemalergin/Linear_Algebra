...
G =  G = listlist2mat([[one,0,one,one],[one,one,0,one],[0,0,0,one],[one,one,one,0],[0,0,one,0],[0,one,0,0],[one,0,0,0]])
...
def find_error(syndrome):
    ...
    Examples:
        ...
        True
    """
    pos = sum([2**(2-i) for i in syndrome.f.keys() if syndrome.f[i] == one]) - 1
    if pos < 0:
